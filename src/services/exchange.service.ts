...
export class ExchangeService {
  ...
  private async mapCryptoInfo(
    ...
    return {
      ...
      lastPrice:
        (isCurrencyThai ? "$ " : "฿ ") +
        parseFloat(lastPrice).toString().replace(regex, ","),
      highPrice:
        (isCurrencyThai ? "$ " : "฿ ") +
        parseFloat(highPrice).toString().replace(regex, ","),
      lowPrice:
        (isCurrencyThai ? "$ " : "฿ ") +
        parseFloat(lowPrice).toString().replace(regex, ","),
      changePrice:
        (changePrice > 0 ? "+" : "") +
        parseFloat(changePrice).toFixed(2).toString().replace(regex, ",") +
        "%",
